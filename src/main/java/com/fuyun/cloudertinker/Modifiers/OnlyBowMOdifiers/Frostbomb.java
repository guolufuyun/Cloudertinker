    ...
    public void arrowhurt(ModifierNBT modifiers, ModDataNBT persistentData, int level, Projectile projectile, EntityHitResult hit, AbstractArrow arrow, LivingEntity attacker, LivingEntity target) {
        if (enabled &&target != null) {
            ...
            if (tooldata.getInt(frostcraft) > 0){
                ...
                List<Mob> mobList = target.getCommandSenderWorld().getEntitiesOfClass(Mob.class, new AABB(x + 2 * a, y + 2 * a, z + 2 * a, x - 2 * a, y - 2 * a, z - 2 * a));
                for (Mob mob : mobList) {
                    if (mob != null) {
                        mob.invulnerableTime= 0;
                        mob.hurt(mob.damageSources().freeze(),(float) (arrow.getBaseDamage() + ((damage * 2 *a)) ));
