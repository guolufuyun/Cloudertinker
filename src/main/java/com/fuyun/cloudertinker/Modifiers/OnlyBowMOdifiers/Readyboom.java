...
public class Readyboom extends BattleModifier {
    ...
    public void arrowhurt(ModifierNBT modifiers, ModDataNBT persistentData, int level, Projectile projectile, EntityHitResult hit, AbstractArrow arrow, LivingEntity attacker, LivingEntity target) {
        ...
        if (target != null &&tooldata.getInt(resentment)>=100) {
            ...
            List<Mob> mobList = target.getCommandSenderWorld().getEntitiesOfClass(Mob.class, new AABB(x + 4, y + 4, z + 4, x - 4, y - 4, z - 4));
            for (Mob mob : mobList) {
                if (mob != null) {
                    MobEffectInstance instance =mob.getEffect((CloudertinkerEffects.ResentmentBrand.get()));
                    if (instance!=null){
                    mob.invulnerableTime= 0;
//                    mob.getCommandSenderWorld().explode(attacker,mob.getX(), mob.getY(), mob.getZ(), 4, true, Explosion.BlockInteraction.NONE);
                        ...
                        for (Mob mob1 : mobList1) {
                            if (mob1 != null) {
                                ...
                                mob1.hurt(mob.damageSources().explosion(attacker,mob),(float) (damage));
