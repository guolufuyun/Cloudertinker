...
public class Brokenimpulse extends BattleModifier {
    ...
    public void afterMeleeHit(IToolStackView tool, ModifierEntry modifier, ToolAttackContext context, float damageDealt) {
        ...
        if (target != null ) {
            ...
            MobEffectInstance instance = target.getEffect(TinkerEffects.bleeding.get());
            ...
            if (instance != null && instance1 == null&&tooldata.getInt(resentment)>=20) {
                int timeleft = target.getEffect(TinkerEffects.bleeding.get()).getDuration();
                int EffectLevel = target.getEffect(TinkerEffects.bleeding.get()).getAmplifier();
                target.invulnerableTime = 0;
                target.hurt(target.damageSources().wither(), 5);
    ...
    public void arrowhurt(ModifierNBT modifiers, ModDataNBT persistentData, int level, Projectile projectile, EntityHitResult hit, AbstractArrow arrow, LivingEntity attacker, LivingEntity target) {
        if (target != null ) {
            ...
            MobEffectInstance instance1 = target.getEffect(CloudertinkerEffects.Evilmarecurse.get());
            if (instance != null && instance1 == null&&tooldata.getInt(resentment)>=20) {
                int timeleft = attacker.getEffect(TinkerEffects.bleeding.get()).getDuration();
                int EffectLevel = attacker.getEffect(TinkerEffects.bleeding.get()).getAmplifier();
                target.invulnerableTime = 0;
                target.hurt(target.damageSources().wither(), 5);
                target.addEffect(new MobEffectInstance(CloudertinkerEffects.Evilmarecurse.get(), timeleft, EffectLevel));
                entitydata.putInt(resentment, entitydata.getInt(resentment)-20);
