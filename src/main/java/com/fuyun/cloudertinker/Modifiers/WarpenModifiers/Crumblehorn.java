    ...
    public void afterMeleeHit(IToolStackView tool, ModifierEntry modifier, ToolAttackContext context, float damageDealt) {
        if (context.getLivingTarget() != null) {
            ...
            if (randomNum <= 7) {
                switch (randomNum) {
                    case 1,2,3:
                        entity.hurt(entity.damageSources().sonicBoom(context.getAttacker()),5 * modifier.getLevel());
                ...
                break;
                    case 4:
                        ...
                        entity.hurt(entity.damageSources().sonicBoom(context.getAttacker()),5 * modifier.getLevel());
                        entity.invulnerableTime = 0;
                        entity.hurt(entity.damageSources().thrown(context.getAttacker(), context.getAttacker()),5 * modifier.getLevel());
                        entity.invulnerableTime = 0;
                        entity.addEffect(new MobEffectInstance(MobEffects.WEAKNESS, 100  ,  modifier.getLevel()-1));
                        entity.addEffect(new MobEffectInstance(CloudertinkerEffects.Armorbroken.get(), 100  ,  modifier.getLevel()-1));
                        entity.addEffect(new MobEffectInstance(TinkerEffects.bleeding.get(),100, modifier.getLevel()-1));
                        entity.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, 100 ,  modifier.getLevel()-1));
                        break;
                    case 5,6,7:
                        entity.hurt(entity.damageSources().thrown(context.getAttacker(), context.getAttacker()),5 * modifier.getLevel());
