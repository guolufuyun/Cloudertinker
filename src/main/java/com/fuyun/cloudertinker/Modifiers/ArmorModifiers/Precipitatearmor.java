...
public class Precipitatearmor extends ArmorModifier {
        ...
        if (entity instanceof ServerPlayer player) {
            if (ModifierLevel.EquipHasModifierlevel(player, this.getId())) {
                if (player.getHealth() <= player.getMaxHealth() * 0.5) {
                    player.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SPEED, 5, (int) (player.getMaxHealth() * 0.5 - player.getHealth() * 0.5)));
                }
            }
        }
    }
